...
bool cmd_vel_active = false, is_running = true;  // Flag indicating cmd_vel priority
ros::Timer cmd_vel_timer;  // Timer to track cmd_vel activity
...
public:
    ...
    void callback(vmxpi_ros_bringup::MotorSpeedConfig &config, uint32_t level) {
        ...
        holonomicDrive(config.linear_x, config.linear_y, config.angular_z);
        publish_motors();
    }
...
int main(int argc, char **argv) {
    ...
    // std::thread thread1(pollButtonInput, button1, 11);
